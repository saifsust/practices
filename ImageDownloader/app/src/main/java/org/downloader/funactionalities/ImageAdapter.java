package org.downloader.funactionalities;
...
/*
 * All urls are stored for downloading image from server
 * Inherite BaseAdapter
 */
public class ImageAdapter extends BaseAdapter {
    ...
    /**
     * All images server links are stored.
     */
    private static final String URLS[] = {"http://lh5.ggpht.com/_mrb7w4gF8Ds/TCpetKSqM1I/AAAAAAAAD2c/Qef6Gsqf12Y/s144-c/_DSC4374%20copy.jpg",
    ...
    /**
     * @return a integer if urls contain link otherwise zero
     */
    ...
    /**
     * @param position is a integer which means which image want to download
     * @return url link to communicate with server for downloading
     */
    @Override
    ...
    /**
     * @param position useed to get specific image.
     * @return long which is a hasCode that are used to find image repidly.
     */
    @Override
    ...
    /***
     * View images in listView in UI.
     * @param position specify the image.
     * @param currView view image in ListView
     * @param parent specify the parent where image will add.
     * @return view to add image into the UI view.
     */

    @Override
    public View getView(int position, View currView, ViewGroup parent) {
        if (currView == null) {
            currView = new ImageView(parent.getContext());
            currView.setPadding(default_padding,default_padding,default_padding,default_padding);
        }
       imageDownloader.download(URLS[position],(ImageView) currView);
        return currView;
    }
